...
enum Command {
    /// Start the Orbiscreen daemon and begin streaming
    Start {
        /// Disable mDNS network service broadcasting
        #[arg(long)]
        no_mdns: bool,
    },
    /// Query the status of the running Orbiscreen daemon
    Status {
        /// Output status information in raw JSON format
        #[arg(long)]
        json: bool,
    },
    /// Stop the running Orbiscreen daemon via D-Bus
    Stop,
    /// Run comprehensive system diagnostics and check compatibility
    Doctor {
        /// Output diagnostic report in JSON format
        #[arg(long)]
        json: bool,
        /// Attempt automatic installation of missing drivers and tools
        #[arg(long)]
        fix: bool,
        /// Answer yes to all interactive prompts during fix
        #[arg(long)]
        yes: bool,
    },
    /// Probe display, capture, and input backends
    Probe,
    /// List configured virtual display properties and monitor specs
    ListDisplays,
    /// Print the active configuration in TOML format
    PrintConfig,
    /// Cleanly remove installed systemd service and local data
    Uninstall,
}
