...
مصفوفة الإصدار: `0.23.4` (مساحة العمل)، `versionCode = 64` (Android). ملاحظة: keystore إصدار Android لم تعد مضمنة في المستودع - راجع SECURITY.md؛ وفّر `ORBISCREEN_KEYSTORE_PATH`/`ORBISCREEN_STORE_PASSWORD`/`ORBISCREEN_KEY_ALIAS`/`ORBISCREEN_KEY_PASSWORD` عند بناء APK الإصدار.
مصفوفة الإصدار: `0.23.5` (مساحة العمل)، `versionCode = 65` (Android). ملاحظة: keystore إصدار Android لم تعد مضمنة في المستودع - راجع SECURITY.md؛ وفّر `ORBISCREEN_KEYSTORE_PATH`/`ORBISCREEN_STORE_PASSWORD`/`ORBISCREEN_KEY_ALIAS`/`ORBISCREEN_KEY_PASSWORD` عند بناء APK الإصدار.
...
عند دفع وسم إصدار (مثلاً `git tag v0.23.4 && git push origin v0.23.4`)، يبني سير العمل `.github/workflows/release.yml` تلقائياً جميع حزم الإصدار ويرفقها بصفحة GitHub Releases.
عند دفع وسم إصدار (مثلاً `git tag v0.23.5 && git push origin v0.23.5`)، يبني سير العمل `.github/workflows/release.yml` تلقائياً جميع حزم الإصدار ويرفقها بصفحة GitHub Releases.
 
يُولَّد محتوى `body` للإصدار من كتلة `## [v0.23.4]` في `CHANGELOG.md`.
يُولَّد محتوى `body` للإصدار من كتلة `## [v0.23.5]` في `CHANGELOG.md`.
