...
[![الإصدار](https://img.shields.io/badge/version-0.25.3-2563eb?style=flat-square&logo=semver)](../CHANGELOG.md)
...
مصفوفة الإصدار: `0.25.3` (مساحة العمل)، `versionCode = 75` (Android). ملاحظة: keystore إصدار Android لم تعد مضمنة في المستودع - راجع SECURITY.md؛ وفّر `ORBISCREEN_KEYSTORE_PATH`/`ORBISCREEN_STORE_PASSWORD`/`ORBISCREEN_KEY_ALIAS`/`ORBISCREEN_KEY_PASSWORD` عند بناء APK الإصدار.
...
عند دفع وسم إصدار (مثلاً `git tag v0.25.3 && git push origin v0.25.3`)، يبني سير العمل `.github/workflows/release.yml` تلقائياً جميع حزم الإصدار ويرفقها بصفحة GitHub Releases.
 
يُولَّد محتوى `body` للإصدار من كتلة `## [v0.25.3]` في `CHANGELOG.md`.
